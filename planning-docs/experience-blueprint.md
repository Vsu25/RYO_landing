...
La ruta ofrece `Explorar / Lista` sobre una sola fuente de platos. En esta etapa la selección se limita a sushi rolls y nigiris. `Explorar` presenta en escritorio una escena encajada de una altura: imagen completa con flechas laterales, categorías en una banda inferior, ficha editorial contigua y selector horizontal de platos. En tablet y teléfono esa misma fuente se transforma en tarjetas desplegables para no encoger la fotografía ni exigir precisión de cursor. `Lista` muestra el mismo contenido como catálogo escaneable. Ningún plato sin fotografía recibe una imagen inventada: el patrón ocupa temporalmente ese espacio. Otras categorías se evalúan únicamente después de la aprobación del cliente.
...
La timeline conserva hitos temporales para entrada, apertura, cada anatomía, cierre y salida, pero no hace snap entre ellos. Cada tramo traduce su progreso a `currentTime` dentro de una timeline virtual compacta; el seek se agrupa y espera al frame presentado antes de enviar el siguiente objetivo al video. El scrub queda en `0.18` para escritorio y `0.14` para tablet; teléfono usa relación directa 1:1 mediante `scrub: true`. El recorrido asigna aproximadamente `12`, `10` y `8.5` alturas de viewport respectivamente: el gesto produce movimiento perceptible sin quitar control sobre las pausas anatómicas. No existe corrección automática de posición ni rebobinado del documento.

La carga audiovisual sigue el mismo orden narrativo. Solo el clip de introducción se solicita al abrir la página. La primera intención de navegación —rueda, toque, puntero o teclado— o el primer 2 % de progreso prepara la apertura y las cuatro capas de rolls; el cierre se prepara antes de su tramo, al superar el 38 %. Así los cambios de calidad o de red no alteran la coreografía y la entrada no compite con media que todavía no puede verse. En teléfono se seleccionan derivados de 960 × 540 px con GOP 6 para evitar decodificar desde el inicio ante cada seek.
