...
| `public/media/` | Diecinueve assets autorizados y publicables, incluidos tres derivados móviles, tres derivados ligeros de marca y el póster WebP crítico de la caja. |
...
La estrategia de descubrimiento reserva `preload="auto"` para la introducción. Apertura y capas de rolls se preparan al primer gesto o al entrar en el primer 2 % de la historia; el cierre, antes de su ventana al 38 %. Sus posters permanecen como `data-poster` hasta ese momento para que tampoco compitan durante la entrada. Los eventos de media sincronizan el `currentTime` pendiente cuando termina la carga, sin refrescar ScrollTrigger por cada archivo. Los tres MP4 de escritorio conservan exactamente su bitstream visual: se retiró únicamente el audio que la experiencia siempre reproduce silenciado. Wordmark, patrón y póster LCP usan derivados WebP dimensionados para su uso real; los masters aprobados permanecen disponibles como fuente.
...
- Dimensiones explícitas, póster inicial WebP y posters diferidos para media todavía invisible.
...
El CSS común permanece en `globals.css`; la coreografía de la landing se importa desde `app/page.tsx` y el sistema del menú desde `app/menu/page.tsx`. El export genera un chunk compartido y uno específico por ruta, evitando enviar a la principal los selectores de catálogo. Durante el scrub, las dos barras de progreso actualizan únicamente su propia transformación; el escenario completo ya no recibe variables CSS por frame. Las lecturas geométricas de la anatomía se agrupan en un solo `requestAnimationFrame` durante resize y desplazamiento táctil.
