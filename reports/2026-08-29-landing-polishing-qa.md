...
- Centro por fase: entrada, giro y final usan el centro real del viewport; durante la anatomía el frame y las capas de puntos/conectores se desplazan juntos `8vw` hasta un centro común en `58vw`, y regresan progresivamente durante el cierre. Verificado a `320 × 568`, `360 × 740`, `375 × 812`, `390 × 844`, `412 × 915` y `430 × 932`, siempre sin overflow horizontal y con una diferencia de centro inferior a `0.01 px`.
- Cierre en viewport compacto: un flick extremo queda devuelto al inicio del tramo final (`scrollY ≈ 5195` en `390 × 844`); el gesto siguiente muestra `Retorno y cierre`, el clip activo en `readyState 4` y `currentTime ≈ 2.43 s`. La comprobación se repitió en el navegador integrado y la continuidad general se inspeccionó también en Safari nativo.
